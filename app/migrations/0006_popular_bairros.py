...
def popular_bairros(apps, schema_editor):
    Bairro = apps.get_model('app', 'Bairro')
    for nome_bairro in BAIRROS_RIO:
        Bairro.objects.get_or_create(nome=nome_bairro)

class Migration(migrations.Migration):

    dependencies = [
        ('app', '0005_bairro_alter_usuario_vehicle_type_and_more'), # IMPORTANTE: Mude '0002' para o número da migração que você anotou no passo A
    ]

    operations = [
        migrations.RunPython(popular_bairros),
    ]